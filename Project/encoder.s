...
// P1 = ^(b11, b9 , b7, b5, b4, b2, b1)
    ...
    XOR_B R3        # R3 = R3 ^ acc =  ^(b11,b9)^(b7, b5, b4, b2, b1) = 0 0 0 0 0 0 0 P1
    LSL R3          # R3 = R3 << 1 = 0 0 0 0 0 0 P[1] 0
    LDA R3          # acc = R3 = 0 0 0 0 0 0 P[1] 0
    ORR R11         # R11 = R11 ^ acc = 0 0 0 P4 0 P2 P1 0
...
// P2 = ^(b11, b10, b7, b6, b4, b3, b1)
    ...
    LSL R3
    LSL R3          # R3  = R3 << 2 = 0 0 0 0 0 P2 0 0 (gets R3 in correct position)
    LDA R3          # acc = R3 = 0 0 0 0 0 P2 0 0
    ORR R11         # R11 = R11 ^ acc = 0 0 0 P4 0 P2 0 0

// P4 = ^(b11:b9, b8, b4, b3, b2)
    LDA R1          # acc = R1 = MEM[1]
    STA R3          # R3 = acc = R1 = MEM[1] = 0 0 0 0  0 b11 b10 b9
    LDA R0          # acc = R0 = MEM[0]
    STA R2          # R2 = acc = R0 = MEM[0] = b8 b7 b6 b5  b4 b3 b2 b1
...
// P0 = ^(b11:b1, p1, p2, p4, p8)
    ...
    LDA R11         # acc = R11 = 0 0 0 P4 0 P2 P1 0 
    STA R2          # R2 = acc = 0 0 0 P4 0 P2 P1 0 
    ...
    ORR R11         # R11 = R11 ^ acc = 0 0 0 P4 0 P2 P1 P0
    
// Construct R11, R12
// R12 =  0 0 0 0 0 0 0 P8;  R11 =  0 0 0 P4 0 P2 P1 P0
    LSL R1
    LSL R1
    LSL R1
    LSL R1
...
// Write data bits in R11, R12 to memory
    LDA R14         # acc = R14 = 30
    ...
    SET_H 0000
    SET_L 0001      # acc = 1
    ADD R14         # R14 += 1 = 32     R14 addr now ready for next iteration
...
// Branch condition: Branch to "Target" if R13 < 30
    SET_H 0001
    SET_L 1110      # acc = 30
